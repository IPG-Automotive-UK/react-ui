...
describe("SidebarItem", () => {
  it("renders name", () => {
    render(<SidebarItem onClick={vi.fn()} name="Inbox" icon={<Mail />} />);
  ...
  it("displays count when present", () => {
    render(
      <SidebarItem onClick={vi.fn()} name="Inbox" icon={<Mail />} count={10} />
    );
    expect(screen.getByText(/9+/i)).toBeInTheDocument();
  });
  it("can be disabled", () => {
    render(
      <SidebarItem onClick={vi.fn()} name="Inbox" icon={<Mail />} disabled />
    );
    expect(screen.getByRole("button")).toHaveClass("Mui-disabled");
  });
  it("can be selected", () => {
    render(
      <SidebarItem onClick={vi.fn()} name="Inbox" icon={<Mail />} selected />
    );
    expect(screen.getByRole("button")).toHaveClass("Mui-selected");
  });
  it("nested children are collapsed by default", () => {
    render(
      <SidebarItem onClick={vi.fn()} name="Parent" icon={<Mail />}>
        <SidebarItem onClick={vi.fn()} name="Child" icon={<Mail />} />
      </SidebarItem>
    );
  ...
  it("nested children can be expanded initially", () => {
    render(
      <SidebarItem onClick={vi.fn()} name="Parent" icon={<Mail />} initialOpen>
        <SidebarItem onClick={vi.fn()} name="Child" icon={<Mail />} />
      </SidebarItem>
    );
