...
describe("WizardStep", () => {
  it("should render the label", () => {
    render(<WizardStep index={0} label="Step 1" />);
    expect(screen.getByText("Step 1")).toBeInTheDocument();
  });
  it("should render the helperText", () => {
    render(
      <WizardStep index={0} label="Step 1" helperText="Step 1 helper text" />
    );
    expect(screen.getByText("Step 1 helper text")).toBeInTheDocument();
  });
  it("should render an error if errorText is provided", () => {
    render(
      <WizardStep index={0} label="Step 1" errorText="Step 1 error text" />
    );
